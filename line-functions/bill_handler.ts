...
📂 ตัดสินใจ category ตามลำดับ:
  ...
  ⚡ Priority 2: น้ำมัน
     "น้ำมัน", "ดีเซล", "แก๊สโซฮอล", "Diesel", "HSD", "ULGSH"
     → category = "ค่าน้ำมัน" (กรอก liters)
...
🗓️ กฎเรื่องวันที่ (สำคัญ — บิลรถมักเป็นบิลใหม่ภายใน 1-2 เดือน):
- ปี 4 หลัก > 2500 = พ.ศ. → ลบ 543 (เช่น 2569 → 2026)
- ปี 4 หลัก < 2500 = ค.ศ. ใช้ตามนั้น
- ปี 2 หลัก:
    * "${yy2}" หรือใกล้เคียง = พ.ศ. ${currentYearBE} → ${currentYear}
    * "${currentYear % 100}" = ค.ศ. ${currentYear} โดยตรง
    * ห้ามตีความเป็น พ.ศ. 25xx เก่า (ใบเสร็จไม่น่าจะเก่าเกิน 1 ปี)
- ⚠️ Sanity check: date ต้องอยู่ใน ±60 วันจาก ${today}
    * ถ้าได้ปี ${currentYear - 10} หรือก่อนหน้า → ปีอ่านผิด ให้เปลี่ยนเป็น ${currentYear}
