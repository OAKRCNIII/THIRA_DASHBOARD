...
export async function tryHandleTruckSelection(
  ...
  if (!plate) {
    // ข้อความที่ "หน้าตาเหมือนทะเบียน" เท่านั้นที่ถือว่าตอบผิด เช่น 72-9999
    // อย่างอื่น (เช่น "คำนวณราคา") ปล่อยผ่านไปให้บอทค่าขนส่งตอบ — บิลค้างยังอยู่ กดปุ่มรถทีหลังได้
    // ไม่งั้นบิลค้างใบเดียวจะทำให้คุยเรื่องอื่นไม่ได้เลย
    if (!/^\s*\d{2}\s*-\s*\d{4}\s*$/.test(text)) return false;

    await reply([{
      type: "text",
      text: `❓ ไม่รู้จักทะเบียน "${text.trim()}" — แตะปุ่มด้านล่าง`,
      quickReply: {
        items: [
          ...truckQuickReplyItems(),
          { type: "action", action: { type: "message", label: "❌ ยกเลิก", text: "/ยกเลิกบิล" } }
...
📂 ตัดสินใจ category ตามลำดับ:
  ...
  ⚡ Priority 2: น้ำมัน
     ...
     🔢 ลำดับความสำคัญในการอ่านยอดเงินน้ำมัน:

     ⚡ กฎเด็ดขาด: ถ้าเห็นรูปแบบ "NNN × NN.NN" (เขียนมือ) → **amount ต้อง = NNN × NN.NN เสมอ**
        คำนวณเอง อย่าเชื่อตัวเลขที่ช่อง TOTAL ถ้ามันไม่ตรงกับผลคูณ
        (ลายมือเลข 0/3/5/8 มักอ่านสับสน — การคูณคือ ground truth)

     【1】 ถ้ามี "NNN × NN.NN" เขียนมือชัดเจน:
            → liters = NNN
            → rate = NN.NN  (ราคาน้ำมันดีเซลไทยปี 2026 ปกติ 30-40 บาท/ลิตร — ถ้าได้ค่านอกช่วงนี้ ให้อ่านใหม่)
            → amount = round(NNN × NN.NN)  (คำนวณเอง อย่าก๊อปจาก TOTAL)
            ตัวอย่าง: "360 x 38.79" → liters=360, amount=13964 (ไม่ใช่ 10164!)
            ตัวอย่าง: "380 x 38.79" → liters=380, amount=14740

     【2】 ถ้าไม่มีการคูณเขียนมือ — มีแต่ "TOTAL/รวม" → ใช้ TOTAL เป็น amount

     【3】 ใบจำกัด/ใบอนุมัติ (ใบสีเหลือง — ไม่มี TOTAL ไม่มี "× ราคา"):
            มีช่อง: DATE / NAME(TR) / CAR No. / LIMIT(ลิตร)
            → plate = ค่าใน CAR No.
            → liters = ตัวเลขในช่อง LIMIT
            → amount = null (บอทจะถามราคา/ลิตรทีหลัง)
            → category = "ค่าน้ำมัน"

     ❌ กฎต้องห้าม:
       - ห้ามตอบ amount=null ถ้าเห็นเลขที่ช่อง TOTAL/รวม/รวมเงิน
       - ห้ามคิดว่าบิลที่มี "NNN x NN.NN" เป็นใบจำกัด — มันคือบิลปกติ
       - ใบจำกัดดูจาก layout "LIMIT" box เท่านั้น ไม่ใช่จากชื่อร้าน
       - บิลวนิดาพานิช = บิลปกติ ต้องมี amount เสมอ
...
❌ ห้ามทำผิดเหล่านี้:
   ...
   - ห้ามอ่านวันสับสน: "03" vs "20", "08" vs "23", "06" vs "26" — เลข 0 ตัวเดียวต้นสุด คือ "0X" ไม่ใช่ "X0"
     ถ้าเลขวันเขียนเป็น "03" (สิบหลักเป็น 0) → วัน = 3 (ไม่ใช่ 20)
     ถ้าเลขวันเขียนเป็น "20" (สิบหลักเป็น 2) → วัน = 20 (ไม่ใช่ 02)
...
ตอบ JSON เท่านั้น ห้ามมีข้อความอื่น`;
  ...
  const res = await callClaudeWithRetry({
    // Sonnet 5 — รับภาพความละเอียดสูง 2576px เท่า Opus 5 (จุดที่ช่วยเรื่องอ่านลายมือ)
    // แต่ราคาไม่ถึงครึ่ง · คิดก่อนตอบเป็นค่าเริ่มต้นเหมือนกัน
    // ถ้าอ่านยังไม่แม่นพอ เปลี่ยนกลับเป็น "claude-opus-5" ได้ทันที ที่เหลือไม่ต้องแก้
